...
from ..data import HubSpotDataLoader, FeatureEngineer, EmployeeRangeOrdinalEncoder, IndustryEncoder , MissingValueHandler
...
class Trainer:
    ...
    def preprocess_data(
        ...
        if fit:
            ...
            self.exp_logger.info(f"Configured: missing_strategy={strategy}, "
    ...
    def train_model(
        ...
        if not hyperparams:
            ...
            hyperparams = {k: v for k, v in vars(self.config.model.hyperparameters).items() 
    ...
    def save_artifacts(self, results: Dict[str, Any]) -> None:
        ...
        if self.config.artifacts.save.model:
            fe_path = self.artifact_dir / self.config.artifacts.models_dir / "feature_engineer.joblib"
        ...
        if self.config.artifacts.save.model:
            ...
            emp_path = self.artifact_dir / self.config.artifacts.models_dir / "employee_encoder.joblib"
            joblib.dump(self.employee_encoder, emp_path)
            
            # Save industry encoder
            ind_path = self.artifact_dir / self.config.artifacts.models_dir / "industry_encoder.joblib"
            joblib.dump(self.industry_encoder, ind_path)
            
            # Save missing handler
            miss_path = self.artifact_dir / self.config.artifacts.models_dir / "missing_handler.joblib"
        ...
        if self.config.artifacts.save.feature_importance:
            try:
                ...
                if importance is not None:
                    feature_names = self.feature_engineer.feature_names if hasattr(self.feature_engineer, 'feature_names') else [f"feature_{i}" for i in range(len(importance))]
                    importance_df = pd.DataFrame({
                        'feature': feature_names,
                        'importance': importance
                    }).sort_values('importance', ascending=False)
                    
                    importance_path = self.artifact_dir / self.config.artifacts.metrics_dir / "feature_importance.csv"
    ...
    def train(
        ...
        with mlflow.start_run(run_name=self.config.experiment.get('run_name', None)):
            ...
            if X is None or y is None:
                ...
                self.exp_logger.info(f"Using pre-loaded data: {X.shape[0]} samples, {X.shape[1]} features")
