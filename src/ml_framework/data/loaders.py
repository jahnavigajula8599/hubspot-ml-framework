...
from .validators import DataDeduplicator, DataQualityProfiler, SchemaValidator
...
class HubSpotDataLoader:
    ...
    def _aggregate_usage_features(self, usage: pd.DataFrame) -> pd.DataFrame:
        ...
        usage_agg.columns = [f"{col[0]}_{col[1]}" for col in usage_agg.columns]
        ...
        usage_agg["activity_frequency"] = usage_agg["WHEN_TIMESTAMP_count"] / (
            usage_agg["days_active"] + 1
        )

        # Drop raw timestamp columns
        usage_agg = usage_agg.drop(
            ["WHEN_TIMESTAMP_count", "WHEN_TIMESTAMP_min", "WHEN_TIMESTAMP_max"], axis=1
        )
    ...
    def get_features_and_target(
        self, df: pd.DataFrame, drop_columns: Optional[list] = None
    ...
    def _validate_customer_mrr(self, customers: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
        ...
        if invalid_count > 0:
            ...
            logger.info(
                f"Saved removed rows to: artifacts/data_quality/removed_invalid_customers.csv"
            )
            ...
            logger.info(
                f"✓ Retained {len(customers_clean)} valid customers ({len(customers_clean)/original_count*100:.2f}%)"
            )
            logger.warning(f"{'='*70}")
