...
class FeatureEngineer:
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> "FeatureEngineer":
        ...
        if self.scaler is not None:
            ...
            scale_features = [
                col for col in self.numeric_features if col not in self.exclude_from_scaling
            ]
...
class IndustryEncoder:
    ...
    def fit(self, X: pd.DataFrame) -> "IndustryEncoder":
        ...
        self.top_industries = industry_counts[industry_counts >= self.min_frequency].index.tolist()
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X["INDUSTRY"] = X["INDUSTRY"].apply(lambda x: x if x in self.top_industries else "Other")
...
class MissingValueHandler:
    ...
    def fit(self, X: pd.DataFrame) -> "MissingValueHandler":
        ...
        if self.fill_values:
            logger.info(
                f"Will impute {len(self.fill_values)} columns with strategy='{self.strategy}'"
            )
