...
class TestBusinessLogic:
    ...
    def test_alexa_rank_reasonable_values(self, all_companies):
        ...
        assert top_100_pct < 5, f"Too many top-100 sites: {top_100_pct:.1f}% (expected < 5%)"
    ...
    def test_actions_must_exceed_users(self, usage_df):
        ...
        assert len(violations) == 0, f"Found {len(violations)} violations of ACTIONS >= USERS rule"
    ...
    def test_action_counts_non_negative(self, usage_df):
        ...
        assert len(violations) == 0, f"Found negative values in {len(violations)} action columns"
...
class TestDataIntegrity:
    ...
    def test_usage_has_timestamps(self, usage_df):
        ...
        """
        assert "WHEN_TIMESTAMP" in usage_df.columns, "Usage data missing WHEN_TIMESTAMP column"
        ...
        assert invalid_pct < 0.1, f"Too many invalid timestamps: {invalid_pct:.2f}%"
    ...
    def test_feature_calculations_on_real_data(self, usage_df):
        ...
        for company_id in usage_df["id"].unique()[:5]:  # Test first 5 companies
            ...
            days_active = (
                company_data["WHEN_TIMESTAMP"].max() - company_data["WHEN_TIMESTAMP"].min()
            ).days
            ...
            if days_active > 0:
                total_actions = (
                    company_data[
                        [col for col in company_data.columns if col.startswith("ACTIONS_")]
                    ]
                    .sum()
                    .sum()
                )
